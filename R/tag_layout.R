...
tag_layout <- function(tag_list,
  ...
  # get x first pair of tags, x being our total well number
  ...
  splitted_sample<- split(as.vector(sample_vec),sort(c(1:length(as.vector(sample_vec)))%%nb_plates)) #each list element a plate. Sorted element A to H column after column (1 to 12)
  ...
  for(i in 1:nb_plates){
    ...
    samplez <- unique(as.vector(as.matrix(PCR_df[[i]]))) # samples can be shorter with duplicate samples when you have triplicate. Requires one tag pairs for multiple wells
